...
def ensure_version_struct(content, values):
    """
    Check if namespace ED_SYSINFO { struct GIT_fwInfo { ... } } exists.
    If not, generate the block and insert it after the last #include.
    """
    # Pattern to detect the struct
    struct_pattern = r'namespace\s+ED_SYSINFO\s*\{\s*struct\s+GIT_fwInfo\s*\{'
...
struct GIT_fwInfo {{
  static constexpr const char *GIT_VERSION = "{values['GIT_VERSION']}";
  static constexpr const char *GIT_TAG = "{values['GIT_TAG']}";
  static constexpr const char *GIT_HASH = "{values['GIT_HASH']}";
  static constexpr const char *FULL_HASH = "{values['FULL_HASH']}";
  static constexpr const char *BUILD_ID = "{values['BUILD_ID']}";
}};
}} // namespace ED_SYSINFO
"""
    ...
    if last_include_idx != -1:
        # Insert after the last #include (one blank line after)
        lines.insert(last_include_idx + 1, '')
        lines.insert(last_include_idx + 2, struct_block)
        return '\n'.join(lines)
    else:
        # No includes? Insert near the top, after any initial comment
...
def update_main_file(filepath, values, submodule_versions):
    ...
    # First, ensure the struct block exists (inject if missing)
    content = ensure_version_struct(content, values)

    # Update Doxygen comment block
    ...
    # Update the static constexpr fields inside the struct
    # We use a regex that matches the fields exactly as they appear inside the struct
    for field, new_value in values.items():
        # Match: static constexpr const char *FIELD = "old_value";
